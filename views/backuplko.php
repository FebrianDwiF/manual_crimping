                            ...
                            <form id="form1" method="post">
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <input type="number" class="form-control" id="qtym" name="qtym" required>
                                    </div>
                                    <div class="col-md-4">
    ...
    <div class="container-field">
                            ...
                            <form id="form2" method="post">
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <input type="number" class="form-control" id="qtym2" name="qtym" required>
                                    </div>
                                    <div class="col-md-4">
                                ...
                                <form id="form3" method="post">
                                    <div class="row g-3">
                                        ...
                                        <div class="col -md-4">
                                            ...
                                            <input type="number" class="form-control" id="qtym3" name="qtym" required>
                                        </div>
                                        <div class="col-md-4">
        ...
        ?>
            ...
            document.addEventListener("DOMContentLoaded", function() {
                ...
                const downtime = document.querySelectorAll('.downtimeForm');
                const dataBody = document.getElementById('dataContainer');

                let currentFormIndex = sessionStorage.getItem('currentFormIndex') ? parseInt(sessionStorage
                    .getItem(
                        'currentFormIndex')) : 0;
                ...
                function showForm(index) {
                    ...
                    downtime.forEach((down, j) => {
                        down.style.display = (j === index) ? 'block' : 'none';
                    });
                ...
                function getLastDowntimeData(formId) {
                    const savedDurations = JSON.parse(sessionStorage.getItem('durations')) || {};
                    console.log("Cek getLastDowntimeData:", savedDurations);

                    return {
                        totalDowntime: savedDurations[formId] || 0,
                        codeError: "ERROR123" // Ganti dengan error yang sesuai jika ada
                    };
                }
                ...
                forms.forEach((form) => {
                    form.querySelector("form").addEventListener("submit", function(event) {
                        ...
                        let formId = form.getAttribute("data-id") ||
                            "defaultID"; // Jika data-id tidak ada, gunakan default


                        console.log("Form ID yang digunakan:", formId);
                        // Ambil ID form
                        const savedDurations = JSON.parse(sessionStorage.getItem(
                            'durations')) || {};
                        const totalDowntime = formatDuration(savedDurations[formId] || 0);

                        console.log("Total downtime untuk Form ID", formId, ":", totalDowntime);


                        let lastDowntimeData = getLastDowntimeData(
                            formId); // Ambil data downtime terakhir
                        console.log("Downtime yang akan dimasukkan ke tabel:",
                            lastDowntimeData);
                        const data = {
                            ...
                            codeError: lastDowntimeData
                                .codeError,
                            downtime: lastDowntimeData
                                .totalDowntime // Tambahkan kode error terakhir
                        };
            ...
            document.addEventListener("DOMContentLoaded", function() {
                ...
                document.querySelectorAll(".downtimeForm").forEach(form => {
                    ...
                    let timerElement = form.querySelector(".timer");
                    let startButton = form.querySelector(".startDowntime");
                    let endButton = form.querySelector(".endDowntime");
                    let dataBody = form.querySelector(".dataBody");
                    let codeErrorInput = form.querySelector(".codeError");
                    let reasonErrorInput = form.querySelector(".reasonError");

                    // Inisialisasi jika belum ada durasi tersimpan
                    if (!durations[formId]) {
                        durations[formId] = 0;
                    }
                    ...
                    function updateTable() {
                        dataBody.innerHTML = "";
                        downtimeRecords[formId]?.forEach((entry, index) => {
                            const row = `<tr>
                    <td>${index + 1}</td>
                    ...
                    startButton.addEventListener("click", function() {
                        if (!timers[formId]) {
                            startTimes[formId] = new Date();
                            timers[formId] = setInterval(() => {
                                let now = new Date();
                                let elapsedMs = now - startTimes[formId] + durations[
                                    formId];
                    ...
                    endButton.addEventListener("click", function() {
                        if (timers[formId] && codeErrorInput.value) {
                            ...
                            let now = new Date();
                            let elapsedMs = now - startTimes[formId] + durations[formId];

                            durations[formId] = elapsedMs;
                            ...
                            let entry = {
                                ...
                                duration: formatDuration(now - startTimes[formId]),
                                totalDowntime: formatDuration(durations[formId])
                            };

                            downtimeRecords[formId] = downtimeRecords[formId] || [];
                            downtimeRecords[formId].push(entry);
                            sessionStorage.setItem("downtimeRecords", JSON.stringify(
                                downtimeRecords));
                            console.log("Durasi downtime tersimpan:", durations);
                            console.log("Data downtime tersimpan:", downtimeRecords);

                            updateTable();
                            timerElement.textContent = "Downtime dihentikan";
